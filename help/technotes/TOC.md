---
audience: end-user
user-guide-title: Notes techniques de Campaign
type: Event
title: Notes techniques d’Adobe Campaign
description: Notes techniques de Campaign
breadcrumb-title: Notes techniques de Campaign
source-git-commit: 25ce962e7c8b6a62fc2c1edb08a78afa839d264e
workflow-type: tm+mt
source-wordcount: '88'
ht-degree: 47%

---


# Notes techniques d’Adobe Campaign {#technotes-ac}

+ [Notes techniques de Campaign](technotes-home.md)
+ Mises à niveau{#tn-new}
   + {hide-from-toc}[Modifications du canal de notification push](upgrades/push-technote.md)
   + [Migrer les utilisateurs et utilisatrices vers IMS](upgrades/migrate-users-to-ims.md)
   + [Migrer un opérateur ou une opératrice technique vers Developer Console](upgrades/ims-migration.md)
   + {hide-from-toc}[Migrer un opérateur ou une opératrice technique vers Developer Console](upgrades/ims-migration-old.md)
   + [Accéder au nouveau connecteur SMS](upgrades/sms-migration.md)
   + {hide-from-toc}[Migration de l&#39;infrastructure d&#39;envoi de Campaign vers AWS](upgrades/migrate-to-aws.md)
   + {hide-from-toc}[Mise à niveau de l&#39;infrastructure d&#39;envoi d&#39;emails de Campaign](upgrades/upgrade-to-aws.md)
   + {hide-from-toc}[Mettre à niveau la version Apache](upgrades/apache.md)
   + {hide-from-toc}[Navigateurs Internet](upgrades/browsers.md)
   + {hide-from-toc}[Mises à niveau du système en 2023](upgrades/tech-stack-upgrade.md)
   + {hide-from-toc}[obsolescence de la console cliente 32 bits](upgrades/console.md)
   + [Schémas 64 bits](upgrades/64-bit-tables.md)
   + Sécurité {#security}
      + {hide-from-toc}[Rotation des informations d’identification du client](security/credential-rotation-guide.md)
      + {hide-from-toc}[&#x200B; Chiffrement asymétrique &#x200B;](security/asymetric-encryption.md)
+ [&lt; Retour à la documentation de Campaign v8](https://experienceleague.adobe.com/fr/docs/campaign/campaign-v8/campaign-home)
