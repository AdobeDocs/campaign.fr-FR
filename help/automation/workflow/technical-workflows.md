...
Adobe Campaign s’accompagne d’un ensemble de workflows techniques intégrés. Ils gèrent les opérations et traitements planifiés pour une exécution périodique sur le serveur. Ils permettent d&#39;effectuer la maintenance sur la base, de transférer les informations de tracking sur les diffusions ou de mettre en place des traitements prévisionnels sur les diffusions. Les workflows techniques sont paramétrés à partir de la **[!UICONTROL Administration > Exploitation > Workflows techniques]** noeud .
...
Les workflows présentés dans cette section sont installés avec les différents packages standard d’Adobe Campaign. Ces packages, ainsi que les workflows techniques associés, dépendent de votre contrat de licence. Les packages natifs sont détaillés dans .
...
Notez que les workflows techniques ne peuvent être démarrés et modifiés que par des opérateurs ayant des permissions d&#39;administration.

>[!NOTE]
>
>Les workflows techniques liés au module complémentaire Message Center sont disponibles par défaut dans la variable **[!UICONTROL Administration]** > **[!UICONTROL Production]** > **[!UICONTROL Message Center]** > **[!UICONTROL Workflows techniques]** noeud .

Découvrez comment surveiller les workflows techniques dans ce [section dédiée](monitor-technical-workflows.md).
...
| **Traitements sur les opérations** (operationMgt) | Installé par défaut | Ce workflow gère les traitements sur les opérations marketing (démarrage du ciblage, extraction des fichiers, etc.). Il crée également les workflows relatifs aux opérations récurrentes et périodiques. |
...
| **Gestion des opt-out NMAC** (mobileAppOptOutMgt) | Canal des applications mobiles (Push) | Ce workflow met à jour les désabonnements aux notifications sur les appareils mobiles. Il se déclenche toutes les 6 heures entre 1h00 et minuit. Voir à ce sujet la section . |
