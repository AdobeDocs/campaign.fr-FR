...
La Carte thermique des workflows permet aux administrateurs de la plateforme Adobe Campaign de surveiller la charge sur l’instance et de planifier les workflows en conséquence. Pour plus d&#39;informations à ce sujet, voir [Surveillance des workflows](heatmap.md).
...
Le nom de l’activité se trouve dans le **[!UICONTROL Avancé]** onglet. Ne les laissez pas nommés **[!UICONTROL requête]**, **[!UICONTROL query1]**, **[!UICONTROL query11]**, mais donnez-leur des noms explicites, par exemple **[!UICONTROL querySubscribedRecipients]**. Ce nom apparaît dans le journal et, le cas échéant, dans les journaux SQL. Cela permet de déboguer le workflow lors de sa configuration.

### Premières et dernières activités {#first-and-last-activities}

* Toujours démarrer votre workflow avec un **[!UICONTROL Démarrer]** une activité ou une **[!UICONTROL Planificateur]** activité. Le cas échéant, vous pouvez également utiliser une **[!UICONTROL Signal externe]** activité.
...
Vous pouvez ajouter du code JavaScript lors de l’initialisation d’une activité de workflow. Cette opération peut être effectuée dans le d’une activité. **[!UICONTROL Avancé]** onglet de l’activité.
...
La plupart du temps, vous ne saurez pas d&#39;où vient le signal. Pour éviter ce problème, utilisez la variable **[!UICONTROL Commentaire]** dans le champ **[!UICONTROL Avancé]** onglet de l’activité signal pour documenter l’origine attendue d’un signal pour cette activité.
...
Seules les tables de travail de la dernière exécution du workflow sont conservées. Les tables de travail des exécutions précédentes sont purgées par le **[!UICONTROL nettoyage]** workflow, qui s’exécute tous les jours.
