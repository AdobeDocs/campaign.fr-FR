...
* **Optimiser la personnalisation d’un email avec des données externes**
...
1. Depuis l’**[!UICONTROL Explorateur de Campaign]**, accédez à **[!UICONTROL Administration]** `>` **[!UICONTROL Plateforme]** `>` **[!UICONTROL Comptes externes]**.

1. Cliquez sur **[!UICONTROL Nouveau]**.

   >[!NOTE]
   >
   > Pour être principal, l’option **[!UICONTROL Activé]** doit être cochée. Si nécessaire, désélectionnez cette option pour désactiver l&#39;accès à cette base de données sans supprimer sa configuration.
...
Tout d&#39;abord, pour que l&#39;utilisateur puisse effectuer des opérations sur une base externe via FDA, l&#39;opérateur doit disposer d&#39;un droit nommé spécifique dans [!DNL Adobe Campaign].
...
1. Saisissez le nom du droit Nommé au format **user:base@server**, où :
   ...
   * **** useris est le nom de l’utilisateur dans la base de données externe
   * **** base est le nom de la base de données externe
   * **** server est le nom du serveur de base de données externe

1. Enregistrez le droit Nommé et associez-le à l&#39;opérateur de votre choix à partir du noeud **[!UICONTROL Administration > Gestion des accès > Opérateurs]** de l&#39;explorateur Adobe Campaign.

Ensuite, pour exploiter les données contenues dans une base externe, l&#39;opérateur Adobe Campaign doit disposer au minimum des permissions &#39;Write&#39; sur la base pour pouvoir créer des tables de travail. Ces tables sont automatiquement supprimées par Adobe Campaign.
...
L&#39;administrateur de la base de données doit faire correspondre ces droits avec les droits spécifiques à chaque moteur de base de données, comme décrit ci-dessous.
...
| **Création de procédures** | N/A | Privilège USAGE ON LANGUAGE python pour pouvoir appeler des scripts Python externes |
...
* **Charger une base de données externe**  : vous pouvez utiliser les données externes dans l&#39;activité  **[!UICONTROL Chargement (SGBD)]** .

* **Ajouter des informations et des liens** - L’activité d’**[!UICONTROL Enrichissement]** permet d’ajouter des données supplémentaires à la table de travail du workflow et de créer des liens vers une table externe. Dans ce contexte, elle peut utiliser des données provenant d’une base de données externe.


Vous pouvez également définir directement une connexion à une base de données externe à partir de ces activités de workflow, pour une utilisation temporaire. Dans ce cas, elle sera stockée dans une base externe locale, réservée à une utilisation dans un workflow en cours : il ne sera pas enregistré sur les comptes externes.
...
1. Sélectionnez les options **[!UICONTROL Données externes]** .
1. Choisissez l&#39;option **[!UICONTROL En définissant localement la source de données]**
1. Sélectionnez le moteur de la base de données cible dans la liste déroulante. Saisissez le nom du server et renseignez les paramètres d&#39;authentification. Indiquez également le nom de la base externe.
