...
* **XS** : cette politique est utilisée pour les tables de taille relativement petite. La table entière est répliquée en une seule fois. La réplication incrémentielle évite de répliquer à plusieurs reprises les mêmes données à l’aide d’un pointeur d’horodatage pour ne répliquer que les modifications récentes.
* **SingleRow** : cette politique ne reproduit qu’une ligne à la fois. Elle est généralement utilisée pour la réplication à la volée impliquant les objets Campaign actuels et les objets associés.
* **SomeRows** : cette politique est conçue pour répliquer un sous-ensemble limité de données à l’aide de définitions de requête ou de filtres. Elle est utilisée pour les tables plus volumineuses où une réplication sélective est nécessaire.
...
|------|-----------|
| **[!UICONTROL Répliquer les tables de référence]** (ffdaReplicateReferenceTables) | Effectue une réplication automatique des tables intégrées qui doivent être présentes dans la base de données locale de Campaign (PostgreSQL) et la base de données cloud ([!DNL Snowflake]). Il est planifié pour s’exécuter toutes les heures, quotidiennement. Si le champ **lastModified** existe, la réplication se produit de manière incrémentielle. Dans le cas contraire, la table entière est répliquée. |
| **[!UICONTROL Répliquer les données d’évaluation]** (ffdaReplicateStagingData) | Réplique les données d’évaluation pour les appels unitaires. Il est planifié pour s’exécuter toutes les heures, quotidiennement. |
...
>[!NOTE]
>
>La réplication à la volée est gérée par des threads techniques spécifiques plutôt que par des workflows. La configuration de ce mode est gérée dans le fichier serverConf.xml. Vous pouvez configurer serverConf.xml pour qu’il corresponde à des cas d’utilisation spécifiques, comme demander que les tables XS soient répliquées de manière incrémentielle plutôt qu’entièrement. Pour plus d’informations, contactez votre représentant ou représentante Adobe.
...
| **Répliquer la file d’attente nmsDlvExclusion** (ffdaReplicateQueueDlvExclusion) | File d’attente pour la table `nms:dlvExclusion`. |
| **Répliquer la file d’attente nmsDlvMidRemoteIdRel** (ffdaReplicateQueueDlvMidRemoteIdRel) | File d’attente pour la table `nms:dlvRemoteIdRel`. |
| **Répliquer la file d’attente nmsTrackingUrl** (ffdaReplicateQueueTrackingUrl)<br/>**Répliquer la file d’attente nmsTrackingUrl simultanément** (ffdaReplicateQueueTrackingUrl_2) | Files d’attente simultanées pour la table `nms:trackingUrl`, utilisant deux workflows pour améliorer l’efficacité en traitant les requêtes en fonction de priorités différentes. |
...
>[!VIDEO](https://video.tv.adobe.com/v/3416865?captions=fre_fr&quality=12)
