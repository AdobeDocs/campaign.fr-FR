...
Cette page répertorie les nouvelles fonctionnalités, les améliorations et les correctifs de la **dernière version de Campaign v8**.
...
<tr>
<td>
<p>La nouvelle activité de workflow <b>Modifier la source de données</b> permet de modifier la source de données de la table de travail d’un workflow. Cela offre une flexibilité améliorée dans la gestion des données entre différentes sources de données (FDA, FFDA et base de données locale).</p>
<p>Dans les workflows Adobe Campaign, les données sont gérées à l’aide de tables de travail (ou temporaires). Au fur et à mesure de l’exécution du workflow, les tables de travail partagent les données entre les activités du workflow. Par défaut, les tables de travail sont créées dans la même base de données que la source des données sur lesquelles nous effectuons des requêtes.</p>
...
<tbody> 
<tr> 
<td> <p>Le canal LINE est désormais disponible avec Campaign v8. Les améliorations suivantes ont été apportées lors de l'utilisation de LINE avec Message Center :
</p>
<ul> 
<li><p>Correction d'une erreur qui pouvait empêcher les visiteurs d'être ciblés dans une diffusion LINE. 
</p></li>
<li><p>Correction d’un problème qui pouvait entraîner des erreurs lors de la récupération des visiteurs de l’instance d’exécution vers l’instance marketing.
</p></li>
<li><p>Correction de problèmes lors du traitement des événements temps réel dans le contexte des diffusions LINE à l'aide de Message Center.</p></li>
</ul>
</td> 
</tr> 
</tbody> 
...
* Correction d’un problème qui empêchait l’affichage du rapport **Hot clicks** pour des diffusions spécifiques.
